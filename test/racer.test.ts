...
import { FreeModelRacer, createRacer, competeFreeModels } from '../src/core/racer.js';
...
describe('FreeModelRacer', () => {
  ...
  describe('race', () => {
    it('should return fastest model response', async () => {
      ...
      const executeWithModel = vi.fn(async (model: string) => {
        ...
        await new Promise(resolve => setTimeout(resolve, delays[model as keyof typeof delays]));
    ...
    it('should reject all models if all fail', async () => {
      ...
      await expect(racer.race(models, executeWithModel, 'test-race-3'))
        .rejects
        .toThrow('All 2 models failed');
    });

    it('should throw if no models provided', async () => {
      await expect(racer.race([], async () => ({} as any), 'test-race-4'))
        .rejects
        .toThrow('No models provided for competition');
    });
    ...
    it('should call onProgress with error when model fails', async () => {
      ...
      expect(onProgress).toHaveBeenCalledWith('failing-model', 'failed', expect.any(Error));
  ...
  describe('raceFromCategory', () => {
    it('should compete all models from category config', async () => {
      ...
      const executeWithModel = vi.fn(async (model: string) => {
        ...
        await new Promise(resolve => setTimeout(resolve, delays[model as keyof typeof delays] || 100));
        return { model, response: 'Response' };
      });

      const result = await racer.raceFromCategory(categoryConfig, executeWithModel);
  ...
  describe('updateConfig', () => {
    ...
    it('should merge new config with existing', () => {
      const originalConfig = racer.getConfig();
